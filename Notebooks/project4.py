...
def processing(data):
    ...
    #Remove urlss
    no_urls = re.sub('((http|https)\:\/\/)?[a-zA-Z0-9\.\/\?\:@\-_=#]+\.([a-zA-Z]){2,6}([a-zA-Z0-9\.\&\/\?\:@\-_=#])*', '', lower)
    # Remove numeric digits    
    no_nums = re.sub('\w*\d\w*', '', no_urls)
    # Remove back slashes
    no_slashes = re.sub("\\\\", "", no_nums)
    # Remove ampersand
    ...
    rep2 = {"nor":"ncr", "but":". But", "pus":"pos", "unfordable":"unaffordable", "alpha":"aloha"}
...
def model_nmf(vectorized_df, categories):
    ...
    return topic_word, nmf_model, doc_topic
